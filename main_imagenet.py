...
parser.add_argument('--lr_drop_epoch_step', default=30, type=int, help='Nr epochs upon which model parameters lr should drop by 0.1')
parser.add_argument('--exp_prefix', default='default_exp', type=str, help='name of experiment')

# Dataset Subset Selection
####################
parser.add_argument('--dataset_subset_ckpt', default=None, type=str, help='path to checkpoint which will be used to generate the subset mask')
parser.add_argument('--subset_frac', default=1.0, type=float, help='Fraction of data to retain in subset')
parser.add_argument('--subset_level', default='dataset', choices=('dataset', 'class'), type=str, help='level at which we will compute subset')
...
def get_train_and_val_loader(args):
    ...
    # Mean and Std normalizations stats
    ...
    train_loader = torch.utils.data.DataLoader(
        HFImageNet(split='train', transform=train_transform,
                    dataset_subset_ckpt= getattr(args, 'dataset_subset_ckpt',  None),
                    subset_level = getattr(args, 'subset_level', None),
                    subset_frac = getattr(args, 'subset_frac', -1)),
    ...
    val_loader = torch.utils.data.DataLoader(
        HFImageNet(split='validation', transform=val_transform),
        batch_size=args.batch_size,
        shuffle=False,
        num_workers=args.workers,
        pin_memory=True)
...
def validate(args, val_loader, model, criterion, epoch, skip_log=False):
    ...
    Args:
        ...
        skip_log (bool): skip logging result on tensorboard if True.
    """
    ...
    with torch.no_grad():
        for i, (inputs, target, _) in tqdm(enumerate(val_loader), total=len(val_loader)):
            if args.gpu is not None:
...
def train_for_one_epoch(args,
    ...
    for i, (inputs, target, index_dataset) in tqdm(enumerate(train_loader), total=total_iterations):
...
def main():
    ...
    args.log_dir = f'./logs_CL/imagenet/{args.exp_prefix}'
    args.save_dir = f'./weights_CL/imagenet/{args.exp_prefix}'
