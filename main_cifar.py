...
parser.add_argument('--pretrained_checkpoint', default=None, type=str, 
    help='pretrained checkpoint we will initialize the model with.')
...
def get_model_and_loss_criterion(args):
    ...
    if args.pretrained_checkpoint:
        ckpt_data = torch.load(args.pretrained_checkpoint)
        model.load_state_dict(ckpt_data['state_dict'])
        print(f'Model weights initialized from: {args.pretrained_checkpoint}')
...
def main_worker(args, config):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % 10 == 0:
            ...
            labels_dict = {'gt_labels': train_loader.dataset.gt_targets,
                      'train_labels': train_loader.dataset.targets}
            utils.save_artifacts(args, epoch, model, class_parameters, inst_parameters, labels_dict)
...
def main():
    ...
    args.log_dir = f'./logs_CL/cifar100/{args.exp_prefix}'
    args.save_dir = f'./weights_CL/cifar100/{args.exp_prefix}'
